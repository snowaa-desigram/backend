...
// Тексты писем: код — отдельной строкой, e2e достаёт его регуляркой [0-9]{6}.
var mailTemplates = map[CodePurpose]struct{ subject, body string }{
	PurposeRegister:      {"Desigram: код подтверждения", "Ваш код подтверждения регистрации: %s\n\nКод действует %d минут."},
	PurposePasswordReset: {"Desigram: сброс пароля", "Ваш код для смены пароля: %s\n\nКод действует %d минут. Если вы не запрашивали сброс — проигнорируйте письмо."},
}
